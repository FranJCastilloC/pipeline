...
def check_table_contents():
    """
    Verifica el contenido de la tabla BB_RFMSOperDia.
    """
    try:
        ...
        # Contar registros
        cursor.execute("SELECT COUNT(*) FROM BB_RFMSOperDia")
        count = cursor.fetchone()[0]
        print(f"\nTotal de registros en la tabla: {count}")
        
        # Mostrar algunos registros
        if count > 0:
            cursor.execute("SELECT TOP 5 * FROM BB_RFMSOperDia ORDER BY fecha DESC")
...
def insert_data(df: pd.DataFrame) -> bool:
    """
    Inserta los datos en la tabla BB_RFMSOperDia usando pyodbc.
    ...
    try:
        ...
        # TODO: Implementar lógica específica para BB_RFMSOperDia
        ...
        for index, row in df.iterrows():
            try:
                # TODO: Implementar MERGE específico para BB_RFMSOperDia
                # cursor.execute("""
                #     MERGE BB_RFMSOperDia AS target
