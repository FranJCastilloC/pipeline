...
def check_table_contents():
    ...
    try:
        ...
        if count > 0:
            cursor.execute("SELECT TOP 5 * FROM BB_ResumenGeneralMercado ORDER BY fecha DESC")
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                print(f"Error en fila {index} ({row['mercado']}, {row['fecha'].strftime('%Y-%m-%d')}): {row_error}")
                skipped_count += 1
                continue
